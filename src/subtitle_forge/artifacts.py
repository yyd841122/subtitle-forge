...
# 执行失败条目的下落措辞（07 票票内裁定）：失败 Source 不产出知识资产
# （含忠实层与发布集，原子作废），其余 Source 不受影响；失败 Source 的
# 重跑语义属 09/11 票，此处不承诺。
GAP_OUTCOME_EXECUTION_FAILURE = "Source 标记为 failed，本次运行不产出其知识资产"

# 解析容忍跳过条目的下落措辞（08 票票内裁定，R1.3）：被跳过的输入行
# 不构成 Segment（其内容不参与提炼——损失如实陈述），本 Source 的处理
# 不受影响、其余可解析内容照常提炼。下落含「不影响处理」（票面措辞），
# 区别于其他三类缺口的下落。
GAP_OUTCOME_PARSE_WARNING = (
    "该行被容忍跳过（不构成 Segment），不影响本 Source 的处理与其余内容的知识提炼"
)
...
class RunSummary:
    ...
    无静默消失——每个 Source 都有实体状态；完成处理的 Source 的全部
    知识单元均有下落记录；失败 Source（07 票）以 failed + 原因对账，
    其已知候选单元逐个记单元级 failed（提炼本身抛错则无已知单元）。
    （R4.3 把作用域升级为"当前资产版本内全量实体"，后续票落地。）
    成本与耗时的可观察记录是其组成部分（A12、R7.1，计量契约由后续
    票展开）。
    """
