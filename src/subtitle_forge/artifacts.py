...
class RunSummary:
    ...
    无静默消失——每个 Source 都有实体状态（07 票：失败 Source 以
    failed + 原因对账，其候选单元整体作废、不留单元级记录）；完成
    处理的 Source 的全部知识单元均有下落记录（R4.3 把作用域升级为
    "当前资产版本内全量实体"，后续票落地），成本与耗时的可观察记录
    是其组成部分（A12、R7.1，计量契约由后续票展开）。
    """
