...
同一接口、可在同一运行中互换（Testing Decisions；13/14 票的接线前提）。
...
不经由生成同一内容的替身自评——03 票落地完整审计门，本票骨架中
审查替身的"通过"判定即当前最小形态。
"""
...
class UnitAuditVerdict:
    ...
    不确定 → 待复核（R4.4 严格语义：无法可靠判定，非低质量兜底）。
    03/04 票在此结构上落地完整判定语义，本票骨架只走"通过"路径。
    """
...
class CoverageVerdict:
    """覆盖审计角色的结论。判定遗漏时缺口报告留"覆盖存疑"（08 票落地）。"""
...
class CognitiveRoles:
    ...
    确定性替身并分别设定行为，真实模型接入（13/14 票）实现同一组接口。
    """
...
class StubInferenceAuditor:
    ...
    拒绝须带理由（缺口报告"审计拒绝"条目的来源，03/04 票展开）。
    """
